...
use yew::{function_component, html, use_effect_with, use_memo, use_mut_ref, use_node_ref, use_state, Html, Properties, WheelEvent};
...
fn draw_edges_in_batches(context: &CanvasRenderingContext2d, graph: Rc<Graph>, drawn_edges: Rc<RefCell<usize>>, scale: f64) {
    ...
    let closure_clone2 = closure_clone.clone();

    *closure_clone.borrow_mut() = Some(Closure::wrap(Box::new(move || {
        let mut edges_drawn = drawn_edges.borrow_mut();
        let end = (*edges_drawn + batch_size).min(total_edges);
        for edge in &graph.edges[*edges_drawn..end] {
            let source = graph.nodes.iter().find(|n| n.key == edge.source);
            let target = graph.nodes.iter().find(|n| n.key == edge.target);
            if let (Some(s), Some(t)) = (source, target) {
                ...
                let edge_color = s.attributes.color.replace("#", "");
                if let Ok(rgb) = u32::from_str_radix(&edge_color, 16) {
                    let r = (rgb >> 16) & 0xFF;
                    let g = (rgb >> 8) & 0xFF;
                    let b = rgb & 0xFF;
                    let rgba = format!("rgba({}, {}, {}, {})", r, g, b, 0.1);
                    context.set_stroke_style_str(&rgba);
                }
                let dx = tx - sx;
                let dy = ty - sy;
                ...
                let curve_offset = (length / 5.0).min(40.0).max(10.0);
    ...
    let closure_timeout = closure_clone2.clone();
    let timeout_closure = Closure::once_into_js(move || {
        window
            .request_animation_frame(
                closure_timeout.borrow().as_ref().unwrap().as_ref().unchecked_ref(),
            )
            .unwrap();
    });
...
pub fn canvas_graph(props: &CanvasGraphProps) -> Html {
    ...
    let graph_rc = use_memo((), |_| {
        let raw = include_str!("../static/graph.json");
        serde_json::from_str::<Graph>(raw)
            .expect("static/graph.json should parse to a Graph")
    });

    {
        ...
        use_effect_with((graph.clone(), width, height, scale, offset_x, offset_y), move |_| {
            if let Some(canvas) = canvas_edges_ref.cast::<HtmlCanvasElement>() {
                ...
                *drawn_edges.borrow_mut() = 0;
                draw_edges_in_batches(&context, graph, drawn_edges, scale.clone());
            }
            || ()
        });
    }
